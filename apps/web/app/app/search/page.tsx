  ...
  return (
    <div style={{ padding: "16px 16px", paddingBottom: 90, maxWidth: 480, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, marginTop: 4 }}>
        ...
        <div style={{ flex: 1, background: "var(--bg-card-alt)", border: "1px solid var(--border-medium)", borderRadius: 14, display: "flex", alignItems: "center", gap: 8, padding: "10px 14px" }}>
          ...
          <input
            ...
            style={{ flex: 1, background: "none", border: "none", color: "var(--text-primary)", fontSize: 15, outline: "none" }}
          />
          {query && (
            <button onClick={() => { setQuery(""); clearResults(); }}
              style={{ background: "none", border: "none", color: "var(--text-faint)", fontSize: 16, cursor: "pointer", padding: 0 }}>✕</button>
          )}
        </div>
      </div>

      {/* Tabs */}
      <div style={{ display: "flex", gap: 4, background: "var(--bg-card)", borderRadius: 12, padding: 3, marginBottom: 16 }}>
      ...
      {tab === "people" && !loading && users.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ fontSize: 12, color: "var(--text-faint)", fontWeight: 700, marginBottom: 4 }}>{users.length} PEOPLE FOUND</div>
          {users.map((u) => (
            <div key={u.id} onClick={() => router.push(`/u/${u.username}`)}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, color: "var(--text-primary)", fontSize: 14 }}>@{u.username}</div>
                {u.full_name && <div style={{ fontSize: 12, color: "var(--text-faint)", marginTop: 1 }}>{u.full_name}</div>}
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 6 }}>
                  {u.fitness_level && (
                    <span style={{ fontSize: 10, color: LEVEL_COLOR[u.fitness_level], background: "var(--bg-page)", borderRadius: 999, padding: "2px 8px", border: `1px solid ${LEVEL_COLOR[u.fitness_level]}44`, fontWeight: 700 }}>
                      {u.fitness_level}
                    </span>
                  )}
                  {u.city && <span style={{ fontSize: 10, color: "var(--text-muted)", background: "var(--bg-page)", borderRadius: 999, padding: "2px 8px", border: "1px solid var(--border-medium)" }}>📍 {u.city}</span>}
      ...
      {tab === "communities" && !loading && communities.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ fontSize: 12, color: "var(--text-faint)", fontWeight: 700, marginBottom: 4 }}>{communities.length} COMMUNITIES FOUND</div>
          {communities.map((c) => (
            <div key={c.id} onClick={() => router.push(`/app/communities/${c.id}`)}
              style={{ background: "var(--bg-card)", borderRadius: 14, padding: "14px 16px", border: "1px solid var(--border)", cursor: "pointer", display: "flex", gap: 12, alignItems: "center" }}>
              <div style={{ width: 46, height: 46, borderRadius: 14, background: "var(--bg-card-alt)", border: "1px solid var(--border-medium)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0 }}>
                {c.avatar_emoji}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, color: "var(--text-primary)", fontSize: 14 }}>{c.name}</div>
                {c.sport && <span style={{ fontSize: 11, color: "var(--accent)", background: "#1a0800", borderRadius: 999, padding: "2px 8px", border: "1px solid var(--accent-faint)", marginTop: 4, display: "inline-block" }}>{c.sport}</span>}
                {c.description && <div style={{ fontSize: 12, color: "var(--text-faint)", marginTop: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.description}</div>}
              </div>
              <span style={{ color: "#333", fontSize: 18 }}>›</span>
            </div>
          ))}
          ...
          {events.map((e) => (
            <div key={e.id} onClick={() => router.push(`/app/events?id=${e.id}`)}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {e.location && <div style={{ fontSize: 11, color: "var(--text-ultra-faint)", marginTop: 2 }}>📍 {e.location}</div>}
              </div>
              <span style={{ color: "#333", fontSize: 18 }}>›</span>
            </div>
          ))}
